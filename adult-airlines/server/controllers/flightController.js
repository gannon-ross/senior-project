...
export async function fetchFlights(req, res) {
    try{
        const { flight_id, flight_number, origin, destination, date } = req.query;

        //Force at least one passed parameter to search
        if (!flight_id && !flight_number && !origin && !destination && !date) {
            return res.status(400).json({message: 'At least one search parameter is required.'});
        }

        //Performs the search if valid input
        const flights = await searchFlights({flight_id, flight_number, origin, destination, date});
